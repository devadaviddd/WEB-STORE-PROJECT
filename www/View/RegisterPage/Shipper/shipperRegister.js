...
btnSubmit.addEventListener('click', (e) => {
    ...
    const profile_picture = document.querySelector("#profile-picture").value;
    const hub = document.querySelector('.form-select');
    ...
    console.log(hub.options[hub.selectedIndex].text);

    const patternOther = new RegExp('^.{5,}$');
    let patternUser = new RegExp('^(?=.*\\d)(?=.*[A-Z])(?=.*[a-z])(.{8,15})$');
    const patternPass = new RegExp('^(?=.*\\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[!@#$%^&*])(.{8,20})$');

    if(username.match(patternUser)  && password.match(patternPass) && (password === password_repeat) && (profile_picture.length != 0) && (hub.selectedIndex != 0) ) {
        console.log("pass");
    } else {
        var notification = document.getElementById("snackbar");

        if(username == "" || password == "" || password_repeat == "" || profile_picture == "") {
